...
class NetworkService {
  ...
  final DoctorMatcherService _matcherService;

  NetworkService({
    FirebaseFirestore? firestore,
    DoctorMatcherService? matcherService,
  })  : _firestore = firestore ?? FirebaseFirestore.instance,
        _matcherService = matcherService ?? DoctorMatcherService();

  Future<Doctor?> getRootUser() async {
    try {
      final docSnapshot = await _firestore
          .collection('users')
          .doc('lUoJzE4iXrVAvJudsCHwUwVjfPB2')
          .get();

      if (!docSnapshot.exists) return null;
      return Doctor.fromSnap(docSnapshot);
    } catch (e) {
  ...
  Future<List<Doctor>> getAllUsers() async {
    try {
  ...
  Future<List<ScoredDoctor>> getTopConnections(
      Doctor user, int limit) async {
    try {
...
class _NetworkVisualizationScreenState
  ...
  Doctor? _rootUser;
  final Map<String, List<ScoredDoctor>> _expandedNodes = {};
  ...
  Future<void> _expandNode(Doctor node) async {
  ...
  void _showUserDetails(BuildContext context, Doctor user,
      [ScoredDoctor? candidate]) {
          ...
          child: Column(
            ...
            children: [
              ...
              if (user.specializations.isNotEmpty)
                Text('Specializations: ${user.specializations.take(2).join(", ")}'),
              if (user.certifications.isNotEmpty)
                Text('Certifications: ${user.certifications.take(2).join(", ")}'),
...
class NetworkVisualizationWidget extends StatefulWidget {
  final Doctor rootUser;
  final Map<String, List<ScoredDoctor>> expandedNodes;
  final Function(Doctor, ScoredDoctor?) onNodeTap;
...
class _NetworkVisualizationWidgetState
  ...
  void _calculatePositions() {
    ...
    widget.expandedNodes.forEach((parentId, connections) {
      ...
      for (var i = 0; i < connections.length; i++) {
        ...
        _nodePositions[connections[i].doctor.id!] = Offset(x, y);
      }
    });
  }
  ...
  List<Widget> _buildNodes() {
    ...
    Widget buildNodeWidget(Doctor user, bool isRoot,
        [ScoredDoctor? candidate]) {
      return Container(
          width: 60,
          height: 60,
    ...
    void addNode(Doctor user, bool isRoot, [ScoredDoctor? candidate]) {
    ...
    widget.expandedNodes.forEach((_, connections) {
      for (final connection in connections) {
        addNode(connection.doctor, false, connection);
      }
    });
  ...
  Widget _buildFallbackAvatar(Doctor user, bool isRoot) {
    return Container(
      color: isRoot ? Colors.blue.shade700 : Colors.blue.shade500,
      child: Center(
        child: Text(
...
class NetworkPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    expandedNodes.forEach((parentId, connections) {
      ...
      for (final connection in connections) {
        final childPos = nodePositions[connection.doctor.id!]!;
...
//   Future<List<ScoredDoctor>> getTopConnections(Doctor user, int limit) async {
//     try {
//       final allUsers = await getAllUsers();
//       final candidates = allUsers.where((u) => u.id != user.id).toList();
//       final matches = await _matcherService.findMatches(user, candidates);
...
// class _NetworkVisualizationWidgetState extends State<NetworkVisualizationWidget> {
//   final Map<String, Offset> _nodePositions = {};
//   double _scale = 1.0;
//   Offset _position = Offset.zero;
...
//   void _showUserDetails(BuildContext context, Doctor user, [ScoredDoctor? candidate]) {
//     final matchScore = candidate?.score ?? 0.0;
//     ScaffoldMessenger.of(context).showSnackBar(
//       SnackBar(
//         duration: const Duration(seconds: 10),
...
//                 shape: BoxShape.circle,
//                 border: Border.all(
//                   color: isRoot ? Colors.green.shade700 : Colors.green.shade500,
//                   width: 3,
//                 ),
//               ),
//               child: CircleAvatar(
//                 backgroundImage: user.photoUrl != null
//                     ? NetworkImage(user.photoUrl!)
//                     : NetworkImage("https://example.com/default-doctor-avatar.png")
//               ),
//             ),
//           ),
//         ),
